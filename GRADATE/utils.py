...
def load_mat(dataset, train_rate=0.3, val_rate=0.1):
    if False:
        ...
        data_mat = sio.loadmat(f'../msgad/data/{dataset}.mat')
        if 'cora' in dataset or 'yelp' in dataset:
            feat = torch.FloatTensor(data_mat['Attributes'].toarray())
        else:
        ...
        label = data_mat['Label'] ; labels = label
        ano_labels = np.squeeze(np.array(label))
        str_ano_labels,attr_ano_labels=ano_labels,ano_labels
        num_node = adj.shape[0]
        num_train = int(num_node * train_rate)
        num_val = int(num_node * val_rate)
        all_idx = list(range(num_node))

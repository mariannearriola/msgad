...
def _plt_plot_graph(G, vertex_color, vertex_size, highlight,
    ...
    if edges and (G.coords.ndim != 1):  # No edges for 1D plots.  # plot edge
    ...
    if G.coords.ndim == 1:
        ...
        # no_high = set(range(30))-set(highlight)
        # no_high = list(no_high)
        # print(no_high)
        print(vertex_size[highlight])
        sc = ax.scatter(*coords_hl.T,
                        c=vertex_color[highlight], s=vertex_size[highlight],
                        marker='.', linewidths=0, alpha=1, zorder=2,  # tjh cmap
                        vmin=limits[0], vmax=limits[1])
        # print(highlight)

        # sc = ax.scatter(*coords_hl.T,
        #                 c=vertex_color[highlight], s=vertex_size*4,
        #                 marker='*', linewidths=0, alpha=1, zorder=2,  # tjh cmap
        #                 vmin=limits[0], vmax=limits[1])

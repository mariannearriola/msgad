...
class Trainer():
	...
	def train(self):
		...
		for e in range(self.args.num_epochs):
			...
			writer.add_scalar('Loss/train', eval_train, e)
			if len(self.splitter.dev)>0 and e>self.args.eval_after_epochs:
				...
				writer.add_scalar('Loss/valid', eval_valid, e)
			...
			if len(self.splitter.test)>0 and eval_valid==best_eval_valid and e>self.args.eval_after_epochs:
				...
				writer.add_scalar('Loss/test', eval_test, e)
	...
	def run_epoch(self, split, epoch, set_name, grad):
		...
		for ind,s in enumerate(split):
			print('\n--- ',str(ind/len(split)),'% done ---\n')
			if self.tasker.is_static:
				s = self.prepare_static_sample(s)
			else:
				s = self.prepare_sample(s)

			print('Number of samples in current batch:',len(s.hist_adj_list))
			...
			loss = 0
			for graph_num in range(len(s.hist_adj_list)):
				adj = s.hist_adj_list[graph_num]
				loss += F.binary_cross_entropy(predictions.view(-1), adj.to_dense().view(-1))
			print('loss:',loss)
	...
	def predict(self,hist_adj_list,hist_ndFeats_list,node_indices,mask_list):
		...
		#for i in range(node_indices.size(1)//predict_batch_size):
		#cls_input = self.gather_node_embs(nodes_embs, node_indices[:, i*predict_batch_size:(i+1)*predict_batch_size])
		cls_input = self.gather_node_embs(nodes_embs, node_indices)

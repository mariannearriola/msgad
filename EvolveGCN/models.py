...
class SGCN(torch.nn.Module):
    def __init__(self,args,activation):
        ...
        w = Parameter(torch.Tensor(args.feats_per_node, args.layer_1_feats))
        self.w_list.append(w)
    
    def forward(self,A_list,Nodes_list,nodes_mask_list):
        node_feats = Nodes_list[-1]
        last_l = node_feats.matmul(self.w_list[0])                                                                       
        '''
...
class Classifier(torch.nn.Module):
    ...
    def forward(self,x):
        #return self.mlp(x)
        transpose = torch.t(x)
        prod = x.matmul(transpose)
        act_out = torch.sigmoid(prod)
        #return torch.sigmoid(x.matmul(torch.t(x)))
        return act_out

...
class GDNLayer(nn.Module):
	...
	def __init__(self, emb_size, cuda, multi_relation=True, num_classes=2):
		"""
        ...
        intra1 = IntraAgg(features, feat_data.shape[1], emb_size, self.dataset['train_pos'], cuda=cuda)
        intra2 = IntraAgg(features, feat_data.shape[1], emb_size, self.dataset['train_pos'], cuda=cuda)
        intra3 = IntraAgg(features, feat_data.shape[1], emb_size, self.dataset['train_pos'], cuda=cuda)
        inter1 = InterAgg(features, feat_data.shape[1], emb_size, self.dataset['train_pos'], self.dataset['train_neg'],
                        adj_lists, [intra1, intra2, intra3], inter=multi_relation, cuda=cuda)

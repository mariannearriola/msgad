...
class Visualizer:
    ...
    def generate_fpath(self,folder):
        fpath = f'vis/{folder}/{self.dataset}/{self.model}/{self.label_type}/{self.exp_name}'
    ...
    def plot_spectrum(self,e,U,signal,color=None):
        c = U.T@signal
        M = torch.zeros((40,c.shape[1])).to(e.device).to(U.dtype)
    ...
    def plot_recons(self,recons_a,recons_labels):
        ...
        for r_ind,r_ in enumerate(recons_a):
            # plot label
            plt.figure()
            e_adj,U_adj= get_spectrum(recons_labels[r_ind].to(self.device).adjacency_matrix().to(torch.float64).coalesce())
            ...
            if len(torch.nonzero(r_symm))==0:
                print(r_ind,'failed')
                import ipdb ; ipdb.set_trace()
            e,U= get_spectrum(r_symm.to_sparse().to(torch.float64))
